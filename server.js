...
client.on('message', msg => {
  ...
  if (msg.guild.channels.find(c => c.name === `destek-${msg.author.id}`)) return msg.author.send(`Zaten Halihazırda Açılmış Bir Destek Talebiniz Bulunmaktadır.`),
    msg.delete()
  }
  ...
  if (msg.channel.name== '『📑』𝐃𝐞𝐬𝐭𝐞𝐤-𝐓𝐚𝐥𝐞𝐩') { 
    ...
    if (!msg.guild.roles.some(r => r.name === "Destek Ekibi")) return msg.author.send(hatay) + msg.guild.owner.send(`${msg.guild.name} Adlı Sunucunda, \`Destek Ekibi\` Adlı Bir Rol Olmadığı İçin Hiçkimse Destek Talebi Açamıyor!`);
    if(msg.guild.channels.find(c => c.name === 'Destek Kanalları')) {
      msg.guild.createChannel(`destek-${msg.author.id}`, "text").then(c => {
      const category = msg.guild.channels.find(c => c.name === 'Destek Kanalları')
      c.setParent(category.id)
      let role = msg.guild.roles.find(r => r.name === "Destek Ekibi");
...
{
    let ever = New.guild.roles.find(r => r.name === "@everyone");
    ...
    if(New.voiceChannelID == voiceID)
    {
        New.guild.createChannel(`konusma-${New.user.id}`,'voice').then(set=>
        {
            ...
            set.overwritePermissions(New.user,
               ...
               /* set.overwritePermissions(kayitsiz,
                {
